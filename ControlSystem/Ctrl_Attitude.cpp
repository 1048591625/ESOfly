...
//	Static_DTCMBuf ESO_AngularRateHeli ESOHeli[3];
	...
	static inline void update_output_throttle( double throttle, double h )
	{
		...
		logbuf[0] = AccENU.z;
		logbuf[1] = AccZ;
		SDLog_Msg_DebugVect("AccZfilter", logbuf , 2 );
			...
			if( throttle > 2.5 ) 
			{
				...
				{	//判断预起飞
					...
					if( sq(CtrlM_ESO[0]->get_EsAngularRate()) + sq(CtrlM_ESO[1]->get_EsAngularRate()) > sq(degree2rad(10.0)) )
						pre_inFlightRP = true;
					if( CtrlM_ESO[2]->get_EsAngularRate() > degree2rad(10.0) )
						pre_inFlightY = true;
				}
				if( AccZ > 20 )
				{	//判断起飞
...
//			WindDisturbance_y += lp_factor * ( WindDisturbance_xy.y - WindDisturbance_y );
			...
			double logbuf[10];
			logbuf[0] = WindDisturbance_xy.x;
			logbuf[1] = WindDisturbance_x;
			logbuf[2] = WindDisturbance_xy.y;
			logbuf[3] = WindDisturbance_y;
			SDLog_Msg_DebugVect( "windTD", logbuf, 4 );
		}
		else
		{
	...
	void update_ESO_1()
	{
		...
		if( Attitude_Control_Enabled )
		{
			CtrlM_ESO[0]->run(angular_rate.x);
			CtrlM_ESO[1]->run(angular_rate.y);
			CtrlM_ESO[2]->run(angular_rate.z);
		}
...
/*机型控制方式*/
	...
	/*多旋翼*/
		...
		{
			//初始化姿态ESO
			((ESO_AngularRate*)CtrlM_ESO[0])->init( cfg.order[0], cfg.T[0], cfg.b[0], cfg.beta[0], cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[1])->init( cfg.order[0], cfg.T[0], cfg.b[2], cfg.beta[0], cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[2])->init( cfg.order2[0], 1.0/(CtrlRateHz*CtrlRateDiv), cfg.b[4], cfg.beta2[0], cfg.beta2[0], 
	...
	/*三轴*/
		static void TriRotor_Init()
		{
			...
			((ESO_AngularRate*)CtrlM_ESO[1])->init( cfg.order[0], cfg.T[0], cfg.b[2], cfg.beta[0], cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[2])->init( cfg.order[0], cfg.T2[0], cfg.b[4], cfg.beta2[0], cfg.beta2[0], 
				cfg.orderD2[0], cfg.betaD2[0],
				CtrlRateHz*CtrlRateDiv );
	...
	/*直升机*/
		...
		static void Heli131_Init()
		{
			...
			((ESO_AngularRateHeli*)CtrlM_ESO[1])->init( cfg.order[0], cfg.T[0], cfg.b[2]*Heli_RPb_scale, cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[2])->init( cfg.order2[0], cfg.T2[0], cfg.b[4], cfg.beta2[0], cfg.beta2[0], 
				cfg.orderD2[0], cfg.betaD2[0],
				CtrlRateHz*CtrlRateDiv );
		}
...
//			ESOHeli[1].init( cfg.T[0], cfg.b[2]*Heli_RPb_scale, cfg.beta[0], CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[0])->init( cfg.order[0], cfg.T2[0], cfg.b[0], cfg.beta[0], cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[1])->init( cfg.order[0], cfg.T2[0], cfg.b[2], cfg.beta[0], cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[2])->init( cfg.order2[0], 1.0/(CtrlRateHz*CtrlRateDiv), cfg.b[4], cfg.beta2[0], cfg.beta2[0], 
				cfg.orderD2[0], cfg.betaD2[0],
...
{	
	...
	for( uint8_t i=0; i<3; ++i )
	{
		if( disturbance[i] > disturbanceMax[i] )
			disturbanceMax[i] = disturbance[i];
		else
			disturbanceMax[i] += DISTURBANCE_MMFILTER_F*h*( disturbance[i] - disturbanceMax[i] );
		if( disturbance[i] < disturbanceMin[i] )
			disturbanceMin[i] = disturbance[i];
		else
			disturbanceMin[i] += DISTURBANCE_MMFILTER_F*h*( disturbance[i] - disturbanceMin[i] );
		...
		disturbance_filted[i] += disturbanceF*h*( disturbance[i] - disturbance_filted[i] );
	}
	
	static vector3<double> last_disturbance = { 0 , 0 , 0 };		
	vector3<double> disturbanceD = (disturbance - last_disturbance) * CtrlRateHz;
	...
	if( pre_inFlightRP || (CtrlM.RP_NoDisturbanceOG==false) )
	{
		switch( CtrlM.RP_ctrl_type )
		{
			...
			{	//控制推力导数
				...
				{
					disturbanceCtrl[0] = disturbance_filted[0];
					disturbanceCtrl[1] = disturbance_filted[1];
				}
				
				outRoll = ( 
						angular_acceleration_ESO.x-disturbanceCtrl[0]
						+ CtrlM_ESO[0]->get_T() * ( angular_acceleration_error.x * P3.x + Ta1.x - disturbanceD[0] ) 
					)/CtrlM_ESO[0]->get_b();
				outPitch = ( 
						angular_acceleration_ESO.y-disturbanceCtrl[1]
		...
		switch( CtrlM.Y_ctrl_type )
		{
			...
			{	//控制推力导数
				...
				if( cfg.Mode[0] & (1<<9) )
					...
					disturbanceCtrl = disturbance_filted[2];
				
				outYaw = ( 
						angular_acceleration_ESO.z-disturbanceCtrl
	...
	{	//起飞前
		switch( CtrlM.Y_ctrl_type )
		{
			...
			case 1:
			{	//期望角加速度P减扰动
				outYaw = ( target_angular_rate_body.z - disturbance_filted[2] ) / CtrlM_ESO[2]->get_b();
				break;
			}
			case 2:
			{	//期望角加速度PD减扰动
...
//	outYaw_filted += 80 * h * ( outYaw - outYaw_filted );
	...
	if( Attitude_Control_Enabled )
	{
		...
		SDLog_Msg_DebugVect( "rol", logbuf, 9 );
		...
		SDLog_Msg_DebugVect( "pit", logbuf, 9 );
...
void init_Ctrl_Attitude()
{
	...
	ParamGroupRegister( "AttCtrl", 3, sizeof(param_types)/sizeof(MAV_PARAM_TYPE), param_types, param_names, (uint64_t*)&cfg );
}